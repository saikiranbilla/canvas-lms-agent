...
export class CanvasClient {
  ...
  async createAnnouncement(courseId: number, title: string, message: string, delayedPostAt?: string): Promise<CanvasAnnouncement> {
    const response = await this.client.post(`/courses/${courseId}/discussion_topics`, {
      title,
      message,
  ...
  async createQuizQuestion(courseId: number, quizId: number, question: any): Promise<any> {
    const response = await this.client.post(

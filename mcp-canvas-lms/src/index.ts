  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        message: { type: "string", description: "Content of the announcement" },
        delayed_post_at: { type: "string", description: "Optional ISO datetime to delay posting" }
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        course_id: { type: "number", description: "ID of the course" },
        quiz_id: { type: "number", description: "ID of the quiz" },
        question_name: { type: "string", description: "Name of the question" },
        question_text: { type: "string", description: "Text of the question (HTML supported)" },
        question_type: { 
          type: "string", 
          enum: ["multiple_choice_question", "true_false_question", "short_answer_question", "essay_question", "matching_question", "numerical_question"],
          description: "Type of the question" 
        },
        points_possible: { type: "number", description: "Points possible for the question" },
        answers: {
          type: "array",
          items: {
            type: "object",
          ...
          description: "List of answers (for multiple choice, etc.)"
        }
      },
      required: ["course_id", "quiz_id", "question_name", "question_text", "question_type"]
    }
  },
      ...
      try {
        ...
        switch (toolName) {
          ...
          case "canvas_create_announcement": {
            const { course_id, title, message, delayed_post_at } = args as {
              course_id: number;
              title: string;
              message: string;
            ...
            const announcement = await this.client.createAnnouncement(course_id, title, message, delayed_post_at);
            return {
          ...
          case "canvas_get_quiz": {
            ...
            if (!course_id || !quiz_id) throw new Error("Missing required fields: course_id and quiz_id");
          ...
          case "canvas_create_quiz": {
            const { course_id, title, ...optionalArgs } = args as { course_id: number; title: string; [key: string]: any };
            if (!course_id || !title) throw new Error("Missing required fields: course_id and title");
            
            const quiz = await this.client.createQuiz(course_id, { title, ...optionalArgs });
            return {
              content: [{ type: "text", text: JSON.stringify(quiz, null, 2) }]
            };
          }
          ...
          case "canvas_start_quiz_attempt": {
            ...
            const session = await this.client.startQuizAttempt(course_id, quiz_id);
            return {
          ...
          case "canvas_create_quiz_question": {
             const { course_id, quiz_id, question_name, question_text, question_type, ...otherArgs } = args as { 
               course_id: number; 
               quiz_id: number;
             ...
             if (!course_id || !quiz_id || !question_name || !question_text || !question_type) {
               throw new Error("Missing required fields: course_id, quiz_id, question_name, question_text, question_type");
             }

             const question = await this.client.createQuizQuestion(course_id, quiz_id, {
               question_name,
               question_text,
               question_type,
               ...otherArgs
